...
export function SentimentDonut({
  ...
  return (
    <div className="flex flex-col items-center gap-3 rounded-xl border border-zinc-200 bg-white p-5">
      <div className="text-sm font-medium text-zinc-900 self-start">Sentiment</div>
      ...
      <ul className="grid w-full grid-cols-2 gap-x-3 gap-y-1.5 self-stretch">
        {segments.map((seg) => (
          <li key={seg.key} className="flex items-center justify-between text-xs text-zinc-600">
            ...
            <span className="tabular-nums text-zinc-900">{seg.pct}%</span>
          </li>
        ))}
      </ul>
    </div>
